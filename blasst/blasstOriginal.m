function [x,varargout] = blasstOriginal(x,lineFrequencies,frequencyRanges,samplingRate,varargin)
...
%         end
        %         BDist = Inf;
        [BDist,~,~] = blasstOriginalTest(reshape(x(jj,:,:),1,size(x,2)*size(x,3)),lineFrequencies(ii),frequencyRanges(ii),samplingRate,scale);
...
%             end
            ...
            showSpectrum(x, samplingRate, {'blasst'}, ['Iteration ' num2str(mm)]);
            
            for kk = 1:size(x,3)
                [yTemp(jj,:,kk),xTemp(jj,:,kk)] = blasstOriginalInternal(x(jj,:,kk),lineFrequencies(ii),frequencyRanges(ii),samplingRate,scale,resolution(ii),(mm-1)*manualOffset);
            ...
            [BDist1,maxFlag,~] = blasstOriginalTest(reshape(xTemp(jj,:,:),1,size(xTemp,2)*size(xTemp,3)),lineFrequencies(ii),frequencyRanges(ii),samplingRate,scale);
            fprintf('BDist = %g, BDist1 = %g\n', BDist, BDist1);
